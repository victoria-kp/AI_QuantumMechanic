"""Infinite Square Well (Particle in a Box) — fully symbolic solve.

Demonstrates the agent's symbolic pipeline for finite-domain problems:
  lookup_equation -> substitute -> solve_ode (with BCs) -> solveset
...
Run from the repo root:
    python -m examples.infinite_square_well
"""
...
# ── Logging helper ───────────────────────────────────────────────────
LOG_FILE = Path(__file__).resolve().parent.parent / "outputs" / "logs" / "infinite_square_well_log.txt"
...
def _log(text: str, *, _file_handle=[None]):
    ...
    if _file_handle[0] is None:
        ...
        _file_handle[0].write("=== Infinite Square Well — Agent Log ===\n\n")
    print(text)
